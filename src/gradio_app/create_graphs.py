...
def create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,label,key):
    ...
    c,_ = scipy.stats.spearmanr(total_changes, ctf_idf_rankings)
    c2,_ = scipy.stats.kendalltau(total_changes, ctf_idf_rankings)
    axes.set_xticklabels(axes.get_xticklabels(), rotation=45, ha='right')
    axes.set_title(f'{label} - Topic {topic_number}\n Spearman rho = {round(c,4)}, Kendall Tau = {round(c2,4)}')
...
def create_graph(path_to_base:str,topic_number:int,choice:int,model:int) -> None : 
    ...
    if choice == 1 : # Total Change
        create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,"Total_Changes","total_changes")
            
    elif choice == 2 :  # Topic Change
        create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,"Topic_Changes","topic_change")
        
    elif choice == 3 :  # Topic to Noise
        create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,"Topic_To_Noise","topic_to_noise")
    
    elif choice == 4 : # All to Noise
        create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,"All_To_Noise","all_to_noise")
        
    elif choice == 5 : # Centroid
        create_graph_base(path_to_base,topic_data,axes,words,ctf_idf_rankings,topic_number,"Topic_Same","topic_same")