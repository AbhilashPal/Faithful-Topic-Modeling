...
def raw_comprehensiveness_checks( docs: List[str], k: int, use_keybert : bool) -> pd.DataFrame:
    """
    Take as input a list of topics and the initial documents, perturbs the documents
    by removing one topic word after another and repeats the modeling to find if the
    topic changes.
    ...
    if use_keybert: 
        ...
        anchor_topic_model = BERTopic(representation_model=representation_model)
    else:
        anchor_topic_model = BERTopic()
    topics, probs = anchor_topic_model.fit_transform(docs)
...
def raw_comprehensiveness_checks_cumulative(docs: List[str], k: int) -> pd.DataFrame:
    """
    Take as input a list of topics and the initial documents, perturbs the documents
    by removing one topic word after another and repeats the modeling to find if the
    topic changes.
...
def dump_comprehensiveness_results(docs: List[str],k:int,path:str,use_keybert:bool):
    ...
    ablation_top_k_topics,c_tf_idf_mappings,df_basic_mapping,topic_list = raw_comprehensiveness_checks(docs,k,use_keybert)
...
def dump_comprehensiveness_results_cumulative(docs: List[str],k:int,path:str):
    """Runs comprehensiveness checks for each of the top k topics formed by Bertopic on the given 
    docs and saves the results in the given path, creating two folders : 
        - Temporary_Results :
            - Base_Results : Containing the base Bertopic Model Results.
    ...
    for topic_i in tqdm(range(k)):
        ablation_top_k_topics[f"Topic_{topic_i}"],c_tf_idf_mappings,df_basic_mapping,topic_list = raw_comprehensiveness_checks_cumulative(docs,topic_i)
    print("========Comprehensiveness Ablation Tests done========")
